---
title: AGR Pro ↔ Collision Component
description: Guide related to AGR Pro Combat Component (in v4) to handle melee collisions
---

Tutorial videos from Angel: [AGR PRO v 4.0.0 Combat Manager Component](https://youtu.be/ls5tQEHiBow) / [Melee weapon full setup](https://youtu.be/vN-_DnaWNvw)
...
Now that collision system for AGR Component is in place, we can test registration of hits. For now, we're going to simply print out information we get from hit results.
...
We have now everything needed to send back that information to other gameplay systems so that they can react to it.